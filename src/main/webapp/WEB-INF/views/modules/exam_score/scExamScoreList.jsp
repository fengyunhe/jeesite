<%@ page contentType="text/html;charset=UTF-8" %>
<%@ include file="/WEB-INF/views/include/taglib.jsp"%>
<html>
<head>
	<title>考试成绩管理</title>
	<meta name="decorator" content="default"/>
	<!-- Add fancyBox -->
    <link rel="stylesheet" href="//cdn.bootcss.com/fancybox/2.1.5/jquery.fancybox.min.css" type="text/css" media="screen" />
    <script type="text/javascript" src="//cdn.bootcss.com/fancybox/2.1.5/jquery.fancybox.pack.js"></script>
	<script type="text/javascript">
		$(document).ready(function() {
            $('.imagelink').fancybox({
                  helpers: {
                      title : {
                          type : 'float'
                      }
                  }
            });
		});
		function page(n,s){
			$("#pageNo").val(n);
			$("#pageSize").val(s);
			$("#searchForm").submit();
        	return false;
        }
	</script>
</head>
<body>
	<ul class="nav nav-tabs">
		<li class="active"><a href="${ctx}/exam_score/scExamScore/">考试成绩列表</a></li>
		<shiro:hasPermission name="exam_score:scExamScore:edit"><li><a href="${ctx}/exam_score/scExamScore/form">考试成绩添加</a></li></shiro:hasPermission>
	</ul>
	<form:form id="searchForm" modelAttribute="scExamScoreVo" action="${ctx}/exam_score/scExamScore/" method="post" class="breadcrumb form-search">
		<input id="pageNo" name="pageNo" type="hidden" value="${page.pageNo}"/>
		<input id="pageSize" name="pageSize" type="hidden" value="${page.pageSize}"/>
		<ul class="ul-form">
			<li><label>学生：</label>
				<form:select path="studentId" cssClass="input-medium" >
					<form:options items="${fns:getStudentList('')}" itemValue="id" itemLabel="name"/>
				</form:select>
			</li>
			<li><label>成绩：</label>
				<form:input path="score" htmlEscape="false" maxlength="11" class="input-medium"/>
			</li>
			<li><label>考试类型：</label>
				<form:select path="examType" class="input-medium">
					<form:option value="" label=""/>
					<form:options items="${fns:getDictList('sc_exam')}" itemLabel="label" itemValue="value" htmlEscape="false"/>
				</form:select>
			</li>
			<li><label>学期：</label>
				<form:select path="semesterId" class="input-medium">
					<form:option value="" label=""/>
					<form:options items="${fns:getDictList('sc_semester')}" itemLabel="label" itemValue="value" htmlEscape="false"/>
				</form:select>
			</li>
			<li><label>年级：</label>
				<form:select path="gradeId" class="input-medium">
					<form:option value="" label=""/>
					<form:options items="${fns:getDictList('sc_grade')}" itemLabel="label" itemValue="value" htmlEscape="false"/>
				</form:select>
			</li>
			<li><label>学科：</label>
				<form:select path="courseId" class="input-medium">
					<form:option value="" label=""/>
					<form:options items="${fns:getDictList('sc_course')}" itemLabel="label" itemValue="value" htmlEscape="false"/>
				</form:select>
			</li>
			<li class="btns"><input id="btnSubmit" class="btn btn-primary" type="submit" value="查询"/></li>
			<li class="clearfix"></li>
		</ul>
	</form:form>
	<sys:message content="${message}"/>
	<table id="contentTable" class="table table-striped table-bordered table-condensed">
		<thead>
			<tr>
				<th>学生</th>
				<th>成绩</th>
				<th>考试类型</th>
				<th>学期</th>
				<th>年级</th>
				<th>学科</th>
				<th>更新时间</th>
				<th>备注信息</th>
				<shiro:hasPermission name="exam_score:scExamScore:edit"><th>操作</th></shiro:hasPermission>
			</tr>
		</thead>
		<tbody>
		<c:forEach items="${page.list}" var="vo">
			<tr>
				<td>
				<a href="${ctx}/exam_score/scExamScore/show?id=${vo.id}">
					${vo.studentId}
				</a>
				</td>
				<td>
				
					${vo.score}
				
				</td>
				<td>
				
					${fns:getDictLabel(vo.examType, 'sc_exam', '')}
				
				</td>
				<td>
				
					${fns:getDictLabel(vo.semesterId, 'sc_semester', '')}
				
				</td>
				<td>
				
					${fns:getDictLabel(vo.gradeId, 'sc_grade', '')}
				
				</td>
				<td>
				
					${fns:getDictLabel(vo.courseId, 'sc_course', '')}
				
				</td>
				<td>
				
					<fmt:formatDate value="${vo.updateDate}" pattern="yyyy-MM-dd HH:mm:ss"/>
				
				</td>
				<td>
				
					${vo.remarks}
				
				</td>
				<shiro:hasPermission name="exam_score:scExamScore:edit"><td>
    				<a href="${ctx}/exam_score/scExamScore/form?id=${vo.id}">修改</a>
					<a href="${ctx}/exam_score/scExamScore/delete?id=${vo.id}" onclick="return confirmx('确认要删除该考试成绩吗？', this.href)">删除</a>
				</td></shiro:hasPermission>
			</tr>
		</c:forEach>
		</tbody>
	</table>
	<div class="pagination">${page}</div>
</body>
</html>